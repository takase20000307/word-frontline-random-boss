# WORD FRONTLINE — 2/3/4 PLAYER & WEAKNESS BOSS

ロイロノートから生徒へ配布できる、ブラウザ完結型の英単語ゲームです。通常学習に加えて、2人・3人・4人のコード式オンライン対戦、約10分の全範囲一斉学習、1分暗記と苦手ボス2周、蓄積データからの苦手一斉学習に対応しています。
...
1. 参加する2人・3人・4人が生徒用URLを開き、「オンライン対戦」を選ぶ。
2. ホストが「2人対戦」「3人対戦」「4人対戦」から選び、「対戦コードを作る」を押す。
3. 表示された6文字をほかの参加者へ伝える。
4. 参加者が同じコードを入力し、全員そろうまで待つ。
5. ホストが10問・20問・30問から問題数を選び、対戦を開始する。

全員に同じ英単語と同じ四択が表示され、正解と回答速度で得点を競います。3人・4人対戦はホストを中心に参加端末が接続するスター型です。氏名、学校名、クラス名は入力・送信しません。自分が間違えた単語は、それぞれの端末の苦手データへ追加されます。
...
- `word-frontline-v2.js`: 2人・3人・4人対戦、10分学習、1分暗記、苦手ボス、苦手一斉学習
...
3. 3人・4人対戦も使う場合は、同じ回線の実機3台または4台で接続を確認する。
